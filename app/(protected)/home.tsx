    ...
    try {
      ...
      if (decisionValue) {
        const donorCloseTime = getClosestDonorClosingTime(
          donorDetails.details.operatingHours
        );
        const recipientNextOpen = getNextRecipientOpenTime(
          recipientDetails.details.operatingHours,
          donorCloseTime
        );
          ...
          >
            <View className="flex-row justify-center items-center my-2.5 mb-8 px-4 gap-3">
              <View className="rounded-xl shadow-lg shadow-black/15 m-2 bg-white/90">
                <LinearGradient
                  colors={["#E8EAF6", "#C5CAE9"]}
                  className="w-full h-full rounded-xl justify-center items-center p-3"
                >
                  <Text className="text-2xl font-bold text-[#303F9F] mb-1">
                    0
                  </Text>
          ...
          <View className="flex-1 bg-black/50 justify-center items-center">
            <View className="m-5 bg-white rounded-2xl p-8 items-center shadow-md shadow-black/25 w-4/5">
              ...
              <View className="flex-row items-center bg-[#F7FAFC] rounded-xl border border-[#E2E8F0] px-4 mb-6 w-full">
                ...
                />
                <Text className="text-base text-[#4A5568] font-medium">
                  sq. ft.
                </Text>
              </View>
            ...
            <View className="m-5 bg-white rounded-2xl p-8 items-center shadow-md shadow-black/25 w-4/5">
              ...
              <View className="w-full mb-4">
                {Object.entries(foodTypes).map(([key, value]) => (
                  <View
                    key={key}
                    className="flex-row items-center mb-3 w-full px-1"
                  >
                    <Checkbox
                      className="mr-3 rounded border-2 border-[#3949AB]"
                      checked={value}
